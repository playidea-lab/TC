...
## 구조 정정 — R1 사서 통합 (2026-05-28 후속 /pi)

위 V1~V14는 v3의 *행동*(에이전트-인-루프, 3-E, cq_dispatch v2)을 정의하지만 *어디서*는 cq-ops 안으로 가정했다. 후속 /pi 메타 토론서 더 근본 결함을 잡음: **4-repo 분리(TC/cq/pcq/cq-ops)는 실제론 3개 개념체**(*도서관+사서* / *실행기* / *규약*)고, cq-ops는 "스킬의 효과적 구현을 위한 *임시* 분리"였는데 explore-loop이 v3로 커지면서(컨트롤러·dispatcher·MCP 서버·봉인) **사서 역할의 서비스급 구현**이 cq-ops로 흘러들어가 정체성을 깬다. autoresearch가 vendored=TC / profile=cq-ops로 쪼개진 게 그 분열의 직접 증상(어제 PR 짝 머지 통증).

**R1 결정**: 사서 역할을 **TC로 통합**한다. V1~V14의 *행동*은 그대로 유지하되 *주거*가 바뀐다:

- **V2 컨트롤러 MCP** → TC `src/the_commons/mcp/exploration.py` (cq-ops 아님).
- **V4 cq_dispatch v2** → TC `src/the_commons/dispatcher/` (어제 cq-ops MR #4 코드 이전).
- **V8 봉인** → TC `src/the_commons/exploration/sandbox.py`.
- **V14 mvtec 마이그**: profile 코드가 TC로 합쳐지면 *마이그가 곧 R1 자체*가 됨 (별도 단계 아님).
- **cq-ops 잔여**: `pi`·`research-loop`·`paper` 영향 없음. `explore-loop` SKILL.md는 TC MCP 호출하는 thin wrapper로 재작성, 신규 `explore-loop-round/`는 매 라운드 절차(D1).
- **TC 정체성 확장**: "증거 도서관 + 매치메이커" → "**연구 운영 플랫폼**"(도서관 + 사서 + QD 자율 운영). R1의 핵심 trade.

**도메인 팩 응집**: autoresearch의 vendored train.py·prepare.py·profile.md·profile.py가 한 자리(`TC/experiments/autoresearch/`)에 모임. cross-repo 짝-PR 머지 통증 영구 해소. mvtec도 동일 패턴.

**보존**: 어제 머지된 PR(cq-ops MR #3, TheCommons PR #1)과 오늘 머지 대기 PR(cq-ops MR #4, PR #2) 자산 *전부 보존*. MR #4의 dispatcher_v2 코드가 B2 단계 이전의 출발점.

상세 이전 계획: `~/.claude/plans/gentle-chasing-fern.md` (Phase A 정리·머지 → B 이전 → C TC MCP 확장 → D v3 본작업).

---
*Generated by /pi on 2026-05-28 — [[explore-loop-autoresearch-profile]] 구현 후 본질 미흡수 자기진단으로부터. R1 구조 정정 2026-05-28(같은 날 후속).*
